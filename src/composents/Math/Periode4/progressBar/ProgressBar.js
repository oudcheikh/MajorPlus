...
const SegmentedProgressBar = ({ totalSegments, currentSegment, setCurrentSegment, progress, setProgress }) => {
    ...
    for (let i = 0; i < totalSegments; i++) {
        segments.push(<span key={i} className={`segment ${i < currentSegment ? "completed" : "incomplete"}`} style={{ width: `${100 / totalSegments}%` }} />);
    }
        ...
        <div>
            <div style={{ display: "flex", alignItems: "center" }}>
                ...
                <div style={{ flex: 1, marginRight: "2px" }}>
                    <div className="segmented-progress-bar">{segments}</div>
                </div>
            </div>
        </div>
    );
